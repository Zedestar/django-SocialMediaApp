...
def user_profile(request, pk):
    ...
    if request.method == "POST":
        ...
        inbox_room = DirectMessageRoom.objects.filter(
            sender=sender, receiver=receiva
        ).first() or DirectMessageRoom.objects.filter(
            sender=receiva, receiver=sender
        ).first()

        # If no room exists, create one
        if not inbox_room:
            inbox_room = DirectMessageRoom.objects.create(sender=sender, receiver=receiva)
...
#     inbox_messages = DirectMessage.objects.filter(room=inbox_room).order_by("-created_at")
#     context = {
