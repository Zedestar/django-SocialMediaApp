...
def home(request):
    ...
    # recent_messages_activity = Messages.objects.filter(room__name__icontains = rooms).order_by("-created_at")[0:5]
    recent_messages_activity = Messages.objects.filter(
        Q(room__topic__icontains = room_filter_query) |
         Q(room__topic__icontains = room_search_query)
        ).order_by("-created_at")[0:15]
